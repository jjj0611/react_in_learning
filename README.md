...
- 이를 'CSS in JS'라고 한다.
- 이것과 관련된 라이브러리 종류가 많은데 https://github.com/MicheleBertoli/css-in-js에서 확인할 수 있다.
- 여기서는 styled-components 라이브러리에 대해 알아보자.

```
yarn add styled-components
```

- components 디렉토리에 StyledButton.js 파일을 만들어보자.
```
...
- styled-components를 불러온 후 styled를 사용하여 Wrapper 값에 스타일링된 div 요소를 만들어주었다.
- 여기서 익숙하지 않은 문법이 나왔는데
```
styled`...`
```
- ES6의 Tagged Template Literals 문법이다.
- 크롬 개발자 도구의 콘솔을 열어 다음 코드를 작성해보자.
```
...
```
- 결과는 [Array(3), 2, 4]이다.
- backquote(`) 사이에 ${자바스크립트 표현}이 들어가면 이처럼 끊어서 함수 인자로 전잘한다.
- 이 문법은 스타일링을 할 때 props에 접근하기 위해 사용한다.

- props 값을 참조해야 할 때는 다음과 같이 함수를 내부에 입력하게 된다.
```
import React from 'react';
import styled from 'styled-components';
...
export default StyledButton;
```
- 이 과정에서 Tagged Template Literals 문법을 사용하지 않으면 다음 함수가 문자열 자체로 들어간다.
- 하지만 해당 문법을 사용하면 자바스크립트 표현을 끊어서 처리하기에 함수가 함수 형태 그대로 배열에 들어간다.
- 이에 따라 styled-components가 끊어진 값들을 차조해서 스타일을 정상적으로 처리할 수 있다.

- styled-components의 최대 장점은 자바스크립트 내부에서 스타일을 정의하기 때문에 자바스크립트와 스타일 사이의 벽이 허물어져 동적 스타일링이 편해진다.
- big이라는 props를 사용하여 버튼 크기를 동적으로 변경하는 코드를 작성해보자.
```
(...)
font-size: ${(props) => props.fontSize};
${props => props.big && `
...
```
- 위 코드에서는 다음 부분이 props를 참조하여 스타일링한다.
```
${props => props.big && `
	font-size: 2rem;
	padding: 2rem;
...
- 이제 App 컴포넌트에서 버튼을 렌더링할 때 big 값을 설정해보자.
...
- 여기에서 <StyledButton big={big}> 이렇게 설정하지 않아도 big이라고만 작성 했는데 의미는 big = {true}와 동일하다.
- styled-components는 이 외에도 많은 기능을 제공한다.
- 예를 들어 Sass에서 사용한 include-media 믹스인처럼 함수를 따로 만들어서 편리하게 반응형 디자인을 할 수도 있고,
- Sass에 감싸인 구조로 스타일을 작성했던 것처럼 styled-components에도 비슷하게 감싸인 구조로 작성이 가능하다.
- 또 현재 선택자 참조(&)도 할 수 있다.
- 더 자세한 내용은 https://www.styled-components.com 을 참고하자.

9.4 정리

- 다양한 방법으로 컴포넌트를 스타일링하는 방법에 대해 배웠다.
- 일반 CSS는 이미 CSS가 익숙한 개발자들에게 친숙하다.
- CSS Module을 사용한다면 스코프 문제가 해결된다.
- 추가로 Sass를 사용하면 더욱 확장된 CSS 문법으로 깔끔하고 편안하게 코드를 작성할 수 있다.
- LESS, Stylus 등도 사용할 수 있다.
- styled-components는 자바스크립트와 CSS의 벽을 허물어주기 때문에 동적 스타일링이 쉽다. 
- 또한 한 파일에서 CSS와 자바스크립트 코드를 모두 관리할 수 있는 장점이 있다.
- 이러한 장단점을 파악하여 방법을 잘 선택하면 된다.
